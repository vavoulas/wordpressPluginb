  ...
  componentDidMount () {
    ...
    if (this.props.duration > 0) {
      ...
      this.dismissTimeout = window.setTimeout(
        ...
        this.props.duration
      ) // then, after the set duration has passed, run the onDismissClick function that has been passed down as a prop from our Admin.jsx container
    ...
    if (this.props.showDismiss) {
      ...
      dismiss = ( // then set the dismiss variable to contain our dismiss button markup
        <span
          ...
          onClick={
            this.props
              .onDismissClick /* when this element is clicked, fire the onDismissClick function that we've passed down as a prop from Admin.jsx */
          }>
      ...
      return (
        // The returned markup uses the standard WordPress notice classes. The resulting notice class and message will depend on the contents of the 'notice' prop object, passed down from Admin.jsx.
        <div
          className={`notice is-dismissible notice-${this.props.notice.type}`}>
          <p>
            <strong>{this.props.notice.message}</strong>
          </p>
          {dismiss}
        </div>
      )
